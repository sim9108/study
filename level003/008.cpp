...
class SimpleOutputBuffer : public basic_streambuf<_Elem, _Traits> {
private:
...
public:
    SimpleOutputBuffer(size_t size = 4) {
        buffer.resize(size);
...
protected:
    int_type overflow(int_type ch) override {
        cout << "[overflow: ";
        for (char* p = this->pbase(); p < this->pptr(); ++p) {
            cout << *p;
        }

        if (!traits_type::eq_int_type(ch, traits_type::eof())) {
            cout << traits_type::to_char_type(ch);
        }
        cout << "]\n";
...
template <class _Elem = char, class _Traits = char_traits<_Elem>>
class NullOutputBuffer : public basic_streambuf<_Elem, _Traits> {
private:
    using char_type = _Elem;
    using traits_type = _Traits;
    using int_type = _Traits::int_type;
...
auto main() -> int {
   do {
        SimpleOutputBuffer myBuffer{};
        ostream out(&myBuffer);
        out << "ABCDE12345";
    } while (false);
